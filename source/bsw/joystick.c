...
/*****************************************************************************/
#define CENTER_POSX 1950  // Actual measured center value for posX
#define CENTER_POSY 1927  // Actual measured center value for posY
/*****************************************************************************/
...
RC_t JOYSTICK_ReadPosition(sint8_t* x, sint8_t* y)
{
   ...
   // Calculate temporary values
    int tempX = (CENTER_POSX - posX) / 16;
    int tempY = (CENTER_POSY - posY) / 16;

    // Clamp values to fit in sint8_t range (-128 to 127)
    if (tempX < -128) tempX = -128;
    else if (tempX > 127) tempX = 127;

    if (tempY < -128) tempY = -128;
    else if (tempY > 127) tempY = 127;

    // Convert to scaled 8-bit signal
    *x = (sint8_t)tempX;
    *y = (sint8_t)tempY;
